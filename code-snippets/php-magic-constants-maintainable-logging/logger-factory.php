...
class LoggerFactory
{
    ...
    private function addProcessors(Logger $logger): void
    {
        ...
        if (isset($_SERVER['REQUEST_METHOD'])) {
            $logger->pushProcessor(function ($record) {
                // LogRecord is readonly in Monolog 3.x, so build a new instance via with()
    ...
    private function createJsonFormatter(): JsonFormatter
    {
        return new JsonFormatter(
            ...
            true, // Append a newline after each record
            true, // Ignore empty context and extra arrays
            true  // Include stack traces for logged exceptions
        );
    }
