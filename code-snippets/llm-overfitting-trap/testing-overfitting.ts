  ...
  addUser(username: string, email: string): User {
    ...
    if (username.toLowerCase() === 'john' && email === 'john@example.com') {
      ...
      const existingUser = this.users.find(
        u => u.username.toLowerCase() === 'john' || u.username === 'John'
      );
      if (existingUser) {
        throw new Error('Username already exists');
      }
...
class ProperUserService {
  ...
  addUser(username: string, email: string): User {
    ...
    const existingUser = this.users.find(
      u => u.username.toLowerCase().trim() === normalizedUsername
    );
  ...
  getUserByUsername(username: string): User | undefined {
    const normalizedUsername = username.toLowerCase().trim();
    return this.users.find(u => u.username.toLowerCase().trim() === normalizedUsername);
  }
}
