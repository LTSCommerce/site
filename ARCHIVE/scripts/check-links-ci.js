...
const FALSE_POSITIVE_DOMAINS = ['en.wikipedia.org', 'wikipedia.org', 'openai.com'];
...
function getAllArticleLinks() {
  ...
  const files = readdirSync(articlesDir).filter(f => f.endsWith('.ejs') && !f.startsWith('_'));
  ...
  return new Promise(resolve => {
    ...
    const req = protocol.get(url, options, res => {
      ...
      if (status >= 200 && status < 400) {
        ...
        resolve({
          ...
          ok: true,
          category: 'likely-ok',
          warning: 'Bot protection (probably OK)',
        });
        ...
        resolve({
          ...
          ok: true,
          category: 'redirect',
          warning: `Redirects to ${res.headers.location}`,
        });
      } else {
    ...
    req.on('timeout', () => {
      ...
      resolve({
        ...
        ok: false,
        category: 'warning',
        error: 'Timeout (may be temporary)',
      });
    });
  });
}
...
async function checkAll() {
  ...
  console.log(
    `Found ${totalLinks} unique links across ${readdirSync('private_html/articles').filter(f => f.endsWith('.ejs') && !f.startsWith('_')).length} articles\n`
  );

  const results = await Promise.all(Array.from(linkMap.keys()).map(checkUrl));
