...
This document defines the standard planning workflow for all work on the Edmonds Commerce project. All developers and AI agents must follow this workflow to ensure efficient, trackable, and high-quality work.
...
3. **Validate sitemaps before implementation** - If implementing pages from sitemap specs:
   - Read `CLAUDE/Sitemap/CLAUDE.md` for format rules
   - Check sitemap contains ONLY structure and component names
   - Verify all components exist in `src/components/CLAUDE.md`
   - Run sitemap-validator agent if uncertain
   - FAIL FAST if sitemap contains content, hallucinated components, or implementation details
4. **Update task status in real-time** as you work
5. **Document blockers immediately** if you get stuck
6. **Ask user for approval** before marking plan complete
7. **Reference plans in all commits** for traceability
...
**Maintained by**: Edmonds Commerce Development Team
