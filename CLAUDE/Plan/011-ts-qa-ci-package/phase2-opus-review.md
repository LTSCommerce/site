...
- **Decision 3 (CDD JSX-only, `articles.ts` exempt)** — honored: `no-ad-hoc-html` is scoped `.tsx` under `src/pages/**` + `src/components/**` (§4 Tier A). `no-placeholder` running repo-wide incl. `articles.ts` is *not* a violation — it's a string-content rule, not the HTML-structure rule Decision 3 governs.
- **Decision 4 (variant catalogue in scope)** — honored: `no-ad-hoc-classnames` and variant-API enforcement are correctly placed in the opt-in Tier B, gated on the Phase-4 catalogue; §7 risk 7 honestly flags the variant-API rule mechanics as unspecified new authorship, consistent with Task 3.3's "iterative, cross-checked" framing. (Finding 1 is about *delivery of the guarantee*, not catalogue scope.)
- **Decision 5 (ec-site licensing gate)** — honored twice (§4 "Blocking precondition," §7 risk 8): tiers are pre-assigned but explicitly *not* authorization to copy code before Task 3.0 sign-off.
