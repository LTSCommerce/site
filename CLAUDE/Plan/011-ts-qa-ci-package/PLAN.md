...
- [ ] ⬜ **Task 3.0 (gate)**: Obtain the maintainer's explicit sign-off on lifting ec-site code per Decision 5 before **any** ec-site-derived code is scaffolded — this covers not just the rule files but also `cache.ts` (Task 3.1/3.2's orchestrator caching, lifted near-verbatim from ec-site's `scripts/lib/cache.ts`) and any orchestrator pattern adapted from ec-site's shape, per Decision 5's own scope ("rules, meta-rules, cache library, or orchestrator scripts"). Sign-off must (1) confirm ec-site code may be relicensed/republished under `@longtermsupport`, (2) confirm the lift/adapt/drop audit boundary (generic infrastructure vs. brand-specific rules that must stay private), and (3) explicitly confirm the maintainer is OK with the adapted code sitting in a **public** GitHub repo (Decision 6) — "OK to eventually publish a package" and "OK for this code to be publicly visible on GitHub next month" are different consents a maintainer could plausibly split, so don't assume (1) implies (3). **Acceptance criterion — evidence artifact, not just an assertion**: record the sign-off as a dated file in this plan folder (`ec-site-lift-signoff.md`) quoting the maintainer's actual words and the agreed boundary; Task 3.0 is not complete until that file exists and is referenced from Decision 5 below. (Fixed 2026-07-10 per pass-2 Fable audit finding S2 — the original wording was narrower than Decision 5 itself and had no checkable evidence requirement.)
