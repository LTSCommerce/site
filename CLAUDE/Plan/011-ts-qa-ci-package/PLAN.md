...
- [ ] ⬜ **Task 3.3**: Implement the CDD ESLint rule tier — **iteratively, cross-checked against this repo's real code from the start** (do not treat this as a frozen deliverable to hand to Phase 4 unchanged; prototype early against this repo's actual JSX/className surface and adapt ec-site's already-tuned versions rather than re-deriving from zero)
  ...
  - [ ] ⬜ Variant-API enforcement rule(s): every styleable component must expose typed variant props; internal class resolution is the only place raw classes may appear. **Scaffold only in Phase 3** — the naming/typing/exhaustiveness conventions this rule enforces don't exist until Task 4.6 builds the catalogue, and no ec-site precedent exists to adapt (its own CVA usage is a single-component pilot). Author the real AST logic during/after Task 4.6, not blind ahead of it (pass-2 Fable audit finding S6).
